...
exports.login = async (req, res) => {
    ...
    const fethedUser = await User.findOne({ where: { email: payload.email }, include: UserRoles })
    const errorResponse = { status: 400, message: 'Invalid email or password' }
    if (!fethedUser) {
        return res.status(400).send(errorResponse);
    }
    const isValidPassword = await cryptor.isValidPassword(payload.password, fethedUser.password)
    if (!isValidPassword) {
...
exports.register = async (req, res) => {
    ...
    if (existingUser) {
       return res.status(400).send({ status: 400, message: 'User already exists with the email' });
    }
